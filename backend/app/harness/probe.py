"""Milestone-0 probe: composition always; live OpenRouter only when opted in.

Default CI must stay green without ``OPENROUTER_API_KEY``, a ``dsh`` binary,
or the optional ``[harness]`` extra. A live round-trip is therefore skip,
not fail, unless ``OPENTHEORY_HARNESS_LIVE`` is set *and* the key + runtime
are present.
"""
...
def live_probe_skip_reason(
    ...
    *,
    ...
    """Return a skip reason, or ``None`` if a live probe may run."""
    ...
    runtime = dsh_available() if dsh_on_path is None else dsh_on_path
    sdk = harness_sdk_available() if sdk_importable is None else sdk_importable
    if not runtime and not sdk:
        return "dsh binary / deepseek-harness-sdk not installed (optional [harness] extra)"
    return None
...
def run_probe() -> ProbeReport:
    ...
    raise RuntimeError(
        "live harness round-trip is not implemented in 0.40.0 — "
        f"pin is {VERSION}; do not invent a settlement path"
    )
