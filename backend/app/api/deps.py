...
async def require_agent_loop_enabled() -> None:
    ...
    Shared by the agent-run, orchestration, and campaign routers so there is
    **one** flag (``AGENT_LOOP_ENABLED``) for the whole agent family. Declared
    as a router-level dependency so it runs
