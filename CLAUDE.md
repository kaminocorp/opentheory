...
**The acting actor.** Every write resolves an `Actor` via the `ActingActor` dependency (`api/deps.py`). Production uses a verified Supabase bearer JWT: token `sub` → `Account` (JIT-provisioned) → that account's primary `human` `Actor` (`0.6.0` auth, `0.7.0` Account-owns-Actor). Local/test may fall back to a dev header when `auth_dev_header_enabled` is on (default **off** in production). A missing/invalid credential is rejected. Write endpoints must declare the dependency rather than inventing an actor.
