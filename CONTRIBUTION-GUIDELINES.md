...
| [`docs/vision/research-git.md`](docs/vision/research-git.md) | The git-for-research ledger semantics. **Target semantics** — operations are annotated *(built)* / *(planned)*: commit, branch, merge, tag, log, semantic diff, and blame are built. |
...
| **Durable / scheduled agent queues** | In-process continuous campaigns shipped in `0.25.0` / concurrent cycles in `0.32.0`. A durable job queue (survive-restart resume) is not built. Autonomy still expands only behind budgets. |
